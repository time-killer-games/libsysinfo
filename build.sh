...
if [ $(uname) = "Darwin" ]; then
  ...
  cd ..; clang++ /opt/local/lib/libSDL2.a gamemaker.cpp system.cpp -o libsysinfo.dylib -Iangle/include -L. -I/opt/local/include -I/opt/local/include/SDL2 -std=c++17 -DCREATE_CONTEXT -lobjc -liconv -lEGL -lGLESv2 -Wl,-framework,CoreAudio -Wl,-framework,AudioToolbox -Wl,-weak_framework,CoreHaptics -Wl,-weak_framework,GameController -Wl,-framework,ForceFeedback -lobjc -Wl,-framework,CoreVideo -Wl,-framework,Cocoa -Wl,-framework,Carbon -Wl,-framework,IOKit -Wl,-weak_framework,QuartzCore -Wl,-weak_framework,Metal -Wl,-weak_framework,OpenGL -fPIC -arch x86_64 -arch arm64 -fPIC -shared; install_name_tool -change @rpath/libEGL.dylib @loader_path/libEGL.dylib ./libsysinfo.dylib; install_name_tool -change @rpath/libGLESv2.dylib @loader_path/libGLESv2.dylib ./libsysinfo.dylib;
