...
if [ $(uname) = "Darwin" ]; then
  ...
  cd ..; clang++ /opt/local/lib/libSDL2.a gamemaker.cpp system.cpp -o libsysinfo.dylib -Iinclude -L. -I/opt/local/include -I/opt/local/include/SDL2 -std=c++17 -DCREATE_CONTEXT -lobjc -liconv -lEGL -lGLESv2 -Wl,-framework,CoreAudio -Wl,-framework,AudioToolbox -Wl,-weak_framework,CoreHaptics -Wl,-weak_framework,GameController -Wl,-framework,ForceFeedback -lobjc -Wl,-framework,CoreVideo -Wl,-framework,Cocoa -Wl,-framework,Carbon -Wl,-framework,IOKit -Wl,-weak_framework,QuartzCore -Wl,-weak_framework,Metal -fPIC -arch x86_64 -arch arm64 -fPIC -shared; install_name_tool -change @rpath/libEGL.dylib @executable_path/libEGL.dylib ./libsysinfo.dylib; install_name_tool -change @rpath/libGLESv2.dylib @executable_path/libGLESv2.dylib ./libsysinfo.dylib;
elif [ $(uname) = "Linux" ]; then
  g++ gamemaker.cpp system.cpp -o libsysinfo.so -std=c++17 -DCREATE_CONTEXT -static-libgcc -static-libstdc++ `pkg-config --cflags --libs sdl2` -lGL -fPIC -shared;
elif [ $(uname) = "FreeBSD" ]; then
  clang++ gamemaker.cpp system.cpp -o libsysinfo.so -std=c++17 -DCREATE_CONTEXT `pkg-config --cflags --libs sdl2` -lGL -lkvm -fPIC -shared;
elif [ $(uname) = "DragonFly" ]; then
  g++ gamemaker.cpp system.cpp -o libsysinfo.so -std=c++17 -DCREATE_CONTEXT -static-libgcc -static-libstdc++ `pkg-config --cflags --libs sdl2` -lGL -lkvm -fPIC -shared;
elif [ $(uname) = "NetBSD" ]; then
  g++ gamemaker.cpp system.cpp -o libsysinfo.so -std=c++17 -DCREATE_CONTEXT -static-libgcc -static-libstdc++ `pkg-config --cflags --libs sdl2` -lGL -fPIC -shared;
elif [ $(uname) = "OpenBSD" ]; then
  clang++ gamemaker.cpp system.cpp -o libsysinfo.so -std=c++17 -DCREATE_CONTEXT `pkg-config --cflags --libs sdl2` -lGL -fPIC -shared;
